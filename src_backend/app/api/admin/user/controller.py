...
from saika.service.forms import AdvancedPaginateForm
...
class AdminUser(DockerAdminAPIController):
    ...
    @get
    @rule('/list')
    @form(AdvancedPaginateForm)
    def list(self):
        pagi = self.service.list(**self.form.data)
    ...
    @rule('/add')
    def add(self):
        self.service.add(**self.form.data)
    ...
    def edit(self):
        if self.form.id.data == self.current_user.id:
            if self.form.role_type.data != RoleShip.TYPE_ADMIN:
                if self.service.count_admin() <= 1:
                    self.error(*SAVE_ONE_ADMIN)

        if self.service.edit(**self.form.data) is False:
            self.error(*USER_NOT_EXISTED)
    ...
    def remove_owner_ship(self):
        ...
        for id in ids:
            try:
                self.service.remove_owner_ship(id)
            except Exception as e:
                excs[id] = e
    ...
    def distribute_object(self):
        ...
        if self.service.distribute_obj(**self.form.data):
